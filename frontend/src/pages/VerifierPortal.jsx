        ...
        try {
            ...
            if (data.status === "Valid") {
                setVerifyData({
                    ...
                    nationalId: data.nationalId || null, // Masked from backend
                ...
                setVerifyData({ merkleRoot: root, leaf: leaf });
                setStatus("revoked");
            } else {
                setStatus("not_found");
            }
                ...
                <div className="space-y-6">
                    ...
                    {(verifyData.studentName || verifyData.degreeTitle || verifyData.institutionName || verifyData.issuedAt) && (
                        <div className="modern-glass-card p-6 rounded-2xl border border-slate-200 space-y-3">
                            ...
                            {verifyData.major && verifyData.major !== 'N/A' && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700">Major</span>
                                    <span className="text-sm font-bold text-slate-900">{verifyData.major}</span>
                                </div>
                            )}
                            {verifyData.minor && verifyData.minor !== 'N/A' && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700">Minor</span>
                                    <span className="text-sm font-bold text-slate-900">{verifyData.minor}</span>
                                </div>
                            )}
                            {verifyData.honors && verifyData.honors !== 'N/A' && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700">Honors / CGPA</span>
                                    <span className="text-sm font-bold text-slate-900">{verifyData.honors}</span>
                                </div>
                            )}
                            ...
                            {verifyData.campus && verifyData.campus !== 'N/A' && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700">Campus</span>
                                    <span className="text-sm font-bold text-slate-900">{verifyData.campus}</span>
                                </div>
                            )}
                            {verifyData.issuedAt && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                            ...
                            {verifyData.placeOfIssue && verifyData.placeOfIssue !== 'N/A' && (
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                    <span className="text-xs font-semibold text-slate-700">Place of Issue</span>
                                    <span className="text-sm font-bold text-slate-900">{verifyData.placeOfIssue}</span>
                                </div>
                            )}
                        </div>
                    )}

                    {/* IDENTITY VERIFICATION WARNING (Answers Panel Critique) */}
                    {verifyData.nationalId && verifyData.nationalId !== 'N/A' && (
                        <div className="modern-glass-card p-6 rounded-2xl border border-slate-200 space-y-3">
                            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Identity Verification</h3>
                            <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                                <span className="text-xs font-semibold text-slate-700">Owner ID (Masked)</span>
                                <span className="text-sm font-bold text-slate-900">{verifyData.nationalId}</span>
                            </div>
                            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-amber-800 text-xs flex items-start gap-2">
                                ...
                                <span>Security Notice: Ensure the last 4 digits of this ID match the candidate's physical ID card to prevent identity theft.</span>
                            </div>
                        </div>
                    )}
